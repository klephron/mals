...
func (s *ScopeController) ModelAcquire(name string, scope *scope.Scope) (string, controller.ScopeToken, error) {
	...
	if !exist {
		...
		s.plane.Infof("%T: new stored %v", s, resource.fullname)
	}
...
func (s *ScopeController) ModelRelease(fullname string, token controller.ScopeToken) error {
	...
	if current == nil {
		err := fmt.Errorf("scope %v path not found", scope)
	...
	if !ok {
		err := fmt.Errorf("resource %v in scope %v not found", name, scope)
		s.plane.Errorf("%T: %v", s, err)
		return err
	}
	...
	s.plane.Infof("%T: %v in scope %v token %v released", s, resource.fullname, scope, token)
...
func (s *ScopeController) scopeClose(errors *[]error, current *stateSpace) {
	current.lsps.Range(func(name string, resource *stateLsp) bool {
		...
		s.plane.Infof("%T: closed lsp %v", s, resource.fullname)
	...
	current.models.Range(func(name string, resource *stateModel) bool {
		...
		s.plane.Infof("%T: close model %v", s, resource.fullname)
